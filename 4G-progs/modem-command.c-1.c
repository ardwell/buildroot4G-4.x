  ...
  // system("kill -s STOP `pidof urc-input`");
...
//    system("kill -s CONT `pidof urc-input`");
    ...
    if(res >= 1)
    {
      ...
      // After sending an AT command the PLS8 immediately sends part 
      // of its expected response, which just so happens is identical to 
      // the message sent, this is the PLS8's echo mode which must be enabled. 

      // There is then a small delay till the PLS8 has completed the request and 
      // has the response ready for the reply.

      // The following code via the while loop and a nanosleep introduces a necessary 
      // delay to ensure that the next call to 'read' will have some characters to read.


      // Up to the point that the OK is received the read(...) should not block indefinitely
