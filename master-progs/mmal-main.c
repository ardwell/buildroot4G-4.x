...
static void camera_control_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
  ...
  {
    MMAL_STATUS_T *param = (MMAL_STATUS_T*)buffer->data;

    printf("[camera_control_callback] Error status <%u>\n", *param);
    /*
    switch(*param)
    {
...
int main(int argc, char** argv) 
{
    ...
    // Find out what type of camera 'object' we are dealing with ...
    printf(" Default camera type <%s>\n", MMAL_COMPONENT_DEFAULT_CAMERA);

    uint32_t nOutputPorts = camera->output_num;
    printf("Number of available output ports <%d>\n", nOutputPorts);
    printf("Number of available input ports <%d>\n", camera->input_num);
    printf("Number of available clock ports <%d>\n", camera->clock_num);
    ...
    format->encoding = MMAL_ENCODING_OPAQUE; // See mmal_encodings.h - image handles are returned and not the actual image data.
    ...
    if (status != MMAL_SUCCESS) 
    {
        printf("Error: Camera failed to enable <%u>\n", status);
        return -1;
    }
